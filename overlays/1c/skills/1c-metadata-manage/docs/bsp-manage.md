...
1. Create processor: `1c-epf-scaffold <Name> -Apply` (dry run without `-Apply`)
...
- **search** / **search_methods** - найти методы модулей SSL для регистрации BSP и проверить точные имена API-методов (замена `ssl_search`).
- **search_objects** - проверить имена целевых объектов метаданных.
- **parse_object_xml** - полная структура целевых объектов для регистрации.
- **search** / **safe_grep** - найти существующие паттерны обработчиков в коде проекта.
