...
1. **First, search SSL** — through RLM (`rlm_execute`): `search(...)` / `search_methods(...)` with keywords describing your need
   - Example: `search("фоновое задание прогресс")`
   - Example: `search("копирование структуры")`

2. **Check existing patterns** — use RLM `search` / `safe_grep` to find how similar tasks are solved in the codebase
