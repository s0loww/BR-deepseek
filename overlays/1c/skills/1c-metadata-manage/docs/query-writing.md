...
1. **Verify metadata** via RLM (`search_objects` / `parse_object_xml`): do the objects, attributes and tabular sections you assume actually exist? Right now? In the version installed in the project?
2. **Find similar queries** via RLM (`search` / `safe_grep`). Reuse a proven shape rather than inventing one.
