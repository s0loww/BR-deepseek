---
description: "Designing 1C registers — dimensions, resources, attributes, periodicity, indexes, balances vs turnovers, posting / reposting / sequence restoration. Load when creating or restructuring an information / accumulation / accounting register. Триггеры: спроектировать регистр, измерения и ресурсы, остатки или обороты, движения документа, индекс регистра."
alwaysApply: false
kind: invariant
---
...
> **Scope.** This file owns *design* rules. XML / schema mechanics live in the 1C metadata-management skill; recurring XML traps — `rules/metadata-xml-workarounds.md`. Anti-patterns of queries against registers — `rules/anti-patterns.md` and `rules/dev-standards-architecture.md §3 → "Queries"`.
...
**Filter virtual tables via parameters, not `ГДЕ` after the call** — hard rule (`rules/anti-patterns.md §4`). Putting the filter into the parameter pushes it into the engine and uses indexes; putting it into `ГДЕ` reads the full virtual table first.

## 8. Posting / reposting

- **`ОбработкаПроведения`** lives in the document's object module. Inside it: lock first, read second, write third (see `rules/locks-and-transactions.md`). Do not call user dialogs, long-running operations, or external services inside the procedure.
...
| Concern | File |
|---|---|
| XML / schema mechanics for register objects | 1C metadata-management skill; XML traps in `rules/metadata-xml-workarounds.md` |
| Query anti-patterns (loops, dot-notation, subselects) | `rules/anti-patterns.md` |
| Authoritative query rules | `rules/dev-standards-architecture.md §3 → "Queries"` |
| Locks during posting | `rules/locks-and-transactions.md` |
| Reporting against registers (DCS) | `rules/dcs-design.md` |
