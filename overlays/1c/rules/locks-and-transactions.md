---
description: "Managed locks, transaction boundaries, lock ordering, deadlock prevention, shared / exclusive lock modes, monitoring via the technological log. Load when designing posting / multi-document operations, debugging lock conflicts, or extending an existing transactional path. Триггеры: конфликт блокировок, взаимоблокировка, deadlock, транзакция не активна, блокировка при проведении."
alwaysApply: false
kind: invariant
---
...
> **Scope.** This file owns the design rules. The narrow case "set a lock before reading balances during posting" lives as a worked example in `rules/platform-solutions.md §9 → "Managed locks and deadlock prevention"` — that section now points back here for the general theory.
...
**Inside an object's `ПередЗаписью` / `ПриЗаписи` / `ОбработкаПроведения`, do NOT open your own `НачалоТранзакции` / `ЗафиксироватьТранзакцию`.** The transaction is already there; a nested call fakes a savepoint but disables proper rollback on the outer error. The symptom to recognise it by («Транзакция не активна» on rollback, partial saves) is indexed in `rules/platform-solutions.md §4`; the treatment is here.
...
The structured payload (`Данные = Структура`) and the dotted event name follow `rules/logging-strategy.md §3-§5`; never log error context as a single concatenated string.
...
- **Diagnose, do not swallow.** Log the error and `ВызватьИсключение` — see `rules/logging-strategy.md §5 → "Error / exception logging"`.
...
Establish a canonical ordering of locked tables and document it in the project's durable knowledge layer (Obsidian project folder or ADR/docs) as a project-wide rule. Example for a typical sales-and-warehouse configuration:
...
See `rules/systematic-debugging.md` for the surrounding methodology — locks must be diagnosed in the Reproduce → Hypothesize → Experiment → Fix order, not by guess-and-retry.

## 7. Companion rules

| Concern | File |
|---|---|
| Worked posting example | `rules/platform-solutions.md §9 → "Managed locks and deadlock prevention"` |
| Transaction nesting in event handlers | `rules/platform-solutions.md §4 → "Transactions in event handlers"` |
| Authoritative query rules under locks | `rules/dev-standards-architecture.md §3 → "Queries"` |
| Logging lock-conflict events | `rules/logging-strategy.md` |
| Systematic debugging of lock incidents | `rules/systematic-debugging.md` |
| Register design (what gets locked) | `rules/registers-design.md` |
