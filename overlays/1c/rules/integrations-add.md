---
description: "when writing code that integrates 1C with another system (HTTP services, REST, message queues, webhooks). Триггеры: обмен с внешней системой, HTTP-сервис, REST, вебхук, токен и авторизация, повторная отправка запроса."
alwaysApply: false
kind: invariant
---
...
- Check whether a ready-made solution already exists in БСП — RLM `search` / `search_methods` over the configuration, which contains БСП (subsystems "Интернет-поддержка пользователей", "Обмен данными", "Получение файлов из Интернета", "Цифровая подпись"). The required scheme is often already implemented.
- Find existing integrations in the configuration — RLM `find_http_services`, `find_web_services`, and `search` for "HTTPСоединение", "ЗаписьJSON", "ЧтениеJSON".
...
- On the client — no synchronous HTTP calls. On `{PLATFORM_VERSION}` ≥ 8.3.21 the platform exposes `Асинх` analogs of the HTTP methods (`ВызватьHTTPМетодАсинх` and friends) — shapes and return values in `async-methods.md`; the `НачатьВыполнение*` callback form (`platform-solutions.md §8`) is the floor for older targets.
...
- JSON — via platform `ЧтениеJSON` / `ЗаписьJSON` (or the equivalent БСП helper if your БСП version provides one — verify the exact name with RLM `search_methods` / `find_exports` before use). Manual string assembly is forbidden.
...
> **Note.** A previous version of this rule suggested prototyping integrations in Python and porting to 1C. That contradicts the root-router project policy: repository code is 1C only. Out-of-1C prototyping is acceptable locally for contract debugging, but Python or other languages do not enter the repository or the production code.
