---
description: "1C Data Composition System (СКД / DCS) design rules — data sets, computed fields vs resources, parameters, settings, variants, programmatic override patterns. Load when deciding how a report should be built or reviewing that decision. Not about running one: if the report already exists and the trouble is memory, speed, or the standard output not fitting the task, that is `dcs-composition-recipes`. Триггеры: как строить отчёт, СКД, вычисляемое поле или ресурс, параметр отчёта, итоги не считаются, набор данных."
alwaysApply: false
kind: invariant
---
...
The 1C Data Composition System (СхемаКомпоновкиДанных, СКД) is the canonical engine for reports. The rules below cover design decisions that recur in code review and that the structural skill (the `1c-metadata-manage` skill (`skd-manage` doc)) intentionally does not opine on.

> **Scope.** This file owns *report design* rules. XML / schema mechanics for `.dcs` files live in the `1c-metadata-manage` skill (`skd-manage` doc) (XML structure, datasets API, query parameters API). Anti-patterns of slow queries inside a DCS — `anti-patterns.md` and `dev-standards-architecture.md §3 → "Queries"`.
...
**Объект in the schema file also costs reference navigation.** The object's fields are untyped, so any composition that navigates through a reference field fails at layout **compile** time («Поле не найдено») — before a single row is read. If the data must be prepared in BSL but the composition still navigates references, keep the schema query-based and substitute the data set at runtime on the compiled layout (`dcs-composition-recipes.md §2`).
...
- **The lookup / set API is asymmetric.** `ПараметрыДанных.НайтиЗначениеПараметра` expects a `ПараметрКомпоновкиДанных` and fails with a type mismatch on a plain string, while `УстановитьЗначениеПараметра` accepts the string. Wrap the name when looking up: `НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяПараметра))`.
...
| Concern | File |
|---|---|
| Going around the standard output (memory, flat export, preprocessing before the fold) | `dcs-composition-recipes.md` |
| XML / schema mechanics for `.dcs` | the `1c-metadata-manage` skill (`skd-manage` doc) |
